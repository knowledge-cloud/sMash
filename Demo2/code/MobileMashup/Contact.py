...
def getJContacts():
	...
	for id in db:
		...
		if db[id].find('last_name'):
			...
			jContact[ID]['last_name']=None
			
		if db[id].find('second_name'):
			test=db[id].find('second_name')[0].value
			test=test.encode('utf-8')
			jContact[ID]['second_name']=test
		else:
			jContact[ID]['second_name']=None
			
		if db[id].find('phone_number'):
			test=db[id].find('phone_number')[0].value
			test=test.encode('utf-8')
			jContact[ID]['phone_number']=test
		else:
			jContact[ID]['phone_number']=None
			
		if db[id].find('mobile_number'):
			test=db[id].find('mobile_number')[0].value
			test=test.encode('utf-8')
			jContact[ID]['mobile_number']=test
		else:
			jContact[ID]['mobile_number']=None

		if db[id].find('email_address'):
			test=db[id].find('email_address')[0].value
			test=test.encode('utf-8')
			jContact[ID]['email_address']=test
		else:
			jContact[ID]['email_address']=None
