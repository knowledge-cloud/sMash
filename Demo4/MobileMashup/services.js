...
function sendXml(){
    this.url = "http://192.168.7.103/MobileMashup/receivexml.php";
    this.send = function(){
...
function MobileGps_getLocation(invokeId) {
    ...
    this.run = function(args) {
        ...
        console.log("MobileGps_getLocation");
        this.handle(null);
    };
    this.handle = function(xmlHttp) {
...
function MobileNetwork_getCellId(invokeId) {
    this.run = function(args) {
        ...
        console.log("MobileNetwork_getCellId");
        this.handle(null);
    };
    this.handle = function(xmlHttp) {
        var results = new Array();
...
function MobileMusic_playMusic(invokeId) {
    this.run = function(args) {
        ...
        console.log("MobileMusic_playMusic");
        if(args != "xx") { // it's running a mashup.
            mainWindow.justMusic = false;
            console.log("a");
        }
        else {
            mainWindow.justMusic = true;
            console.log("b");
        }
...
function GoogleStaticMap_addMarkers(invokeId) {
	this.url = "http://maps.google.com/maps/api/staticmap?size=325x325&maptype=roadmap&markers=color:green|";
	this.run = function(args){
		this.url += args['lat'] + ',' + args['lon'] + "&sensor=false&zoom=12";
		this.handle(null);
	};
...
function Flickr_getPhotos(invokeId) {
    this.url = "http://www.flickr.com/services/rest/?method=flickr.photos.search"+
         ...
         "&sort=interestingness-desc&per_page=5&text=Cher";
         
    this.run = function(args) {
    	   //this.url += "&text=" + args.search_text;
    	  myConsole.log += "Flickr_getPhotos\n";
        environment.getRequest(this, this.url);
    };
 ...
 * example of url: http://oyyt.com/manage.php
 */
function Proxy_getResult(invokeId) {
    this.url = "http://192.168.7.103/MobileMashup/manage.php?invokeid=" + invokeId + "&";
	...
	this.run = function(args) {
		...
		myConsole.log += argss + "\n";
		environment.getRequest(this, this.url + argss);
	};
    ...
    this.handle = function(xmlHttp) {
        ...
        var data = datas.data;
        for(var invokeid in data){
        		var results = this._getArrayFromJSON(data[invokeid]);
        		mashup.context[invokeid] = results;
        }
        engine.insertResult(new Array());
        engine.resume();
        //engine._waiting = false;
        //engine.finish();
    };

    this._getArrayFromJSON = function(datas) {
    ...
    function _Result()
    {
	        ...
	        for(var key in data){
	        		eval("this." + key + "=new _data('" + data[key] + "')");
	        }
        };
    };
    ...
    this.error = function(){
    		myConsole.log += "proxy conn timeout!\n";
    		engine.retryInLocal();
    		engine.insertResult(new Array());
         engine.resume();
    };
...
/*
 * service: Proxy_checkProxy
 * example of url: http://oyyt.com/checkproxy.php
 */
function Proxy_checkProxy(invokeId) {
    this.url = "http://192.168.7.103/MobileMashup/checkproxy.php"
    ...
    this.handle = function(xmlHttp) {
        ...
        myConsole.log += "proxy is ready!\n";
        engine.back2Cloud();
    };
    
    this.error = function(){
    		myConsole.log += "still no proxy!\n";
    };
}