...
var engine = {
  ...
  //      myConsole.log += "Start send xml\n";
    ...
    retryInLocal: function(){
    		myConsole.log += "retryInLocal()\n";
    		printList(this.currentActivity);
    		this.currentActivity = changeListCloud2Local(this.currentActivity);
    		printList(this.currentActivity);
    },
    
    back2Cloud: function(){
    		myConsole.log += "back2Cloud()\n";
    		printList(this.currentActivity);
    		this.currentActivity.next = changeListLocal2Cloud(this.currentActivity.next);
    		printList(this.currentActivity);
    }
};
...
function executeInvoke(invoke){
    ...
    for (var j = 0; j < invoke.inputValues.length; j++) {
    		//myConsole.log += "input."+invoke.inputValues[j][0]+"=\""+evaluateExpression3(invoke.inputValues[j][1], invoke.parent)+"\"";
...
/**
 * change the execute list from cloud to local
 * args head: head of the list.
 * return: head of the changed list.
 */
function changeListCloud2Local(head){
	if(head == null || !(head instanceof Invoke) || head.origin == null){
		return head;
	}
	head.next = head.origin;
	return head;
}


/**
 * for debug. print the id of the list.
 */
function printList(head){
	var node = head;
	while(node != null){
		myConsole.log += node.id + "\n";
		node = node.next;
	}
	myConsole.log += "end\n"
}